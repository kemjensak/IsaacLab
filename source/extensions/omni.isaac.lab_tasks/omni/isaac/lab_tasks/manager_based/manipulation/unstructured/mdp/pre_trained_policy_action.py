...
class PreTrainedPolicyAction(ActionTerm):
    ...
    def __init__(self, cfg: PreTrainedPolicyActionCfg, env: ManagerBasedRLEnv) -> None:
        ...
        # load policy
        if not (check_file_path(cfg.grasp_policy_path)):
            raise FileNotFoundError(f"Policy file '{cfg.grasp_policy_path}' does not exist.")
        if not (check_file_path(cfg.flip_policy_path)):
            raise FileNotFoundError(f"Policy file '{cfg.flip_policy_path}' does not exist.")
        file_bytes = read_file(cfg.grasp_policy_path)
        self.grasp_policy = torch.jit.load(file_bytes).to(env.device).eval()
        file_bytes = read_file(cfg.flip_policy_path)
        self.flip_policy = torch.jit.load(file_bytes).to(env.device).eval()
...
class PreTrainedPolicyActionCfg(ActionTermCfg):
    ...
    grasp_policy_path: str = MISSING
    flip_policy_path: str = MISSING
